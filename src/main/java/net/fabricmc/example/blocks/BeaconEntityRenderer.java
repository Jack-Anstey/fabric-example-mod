...
public class BeaconEntityRenderer {//implements BlockEntityRenderer<BeaconEntity> {
    /*
    public static final Identifier BEAM_TEXTURE = new Identifier("textures/entity/beacon_beam.png");

    public BeaconEntityRenderer(BlockEntityRendererFactory.Context ctx) {
    }

    public void render(BeaconEntity beaconEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        long l = beaconEntity.getWorld().getTime();
        List<BeaconEntity.BeamSegment> list = beaconEntity.getBeamSegments();
        int k = 0;

        for(int m = 0; m < list.size(); ++m) {
            BeaconEntity.BeamSegment beamSegment = list.get(m);
    ...
    public boolean rendersOutsideBoundingBox(BeaconEntity beaconEntity) {
        return true;
    }
    ...
    public boolean method_33892(BeaconEntity beaconBlockEntity, Vec3d vec3d) {
